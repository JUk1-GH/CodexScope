...
它是一个静态 HTML 页面：没有后端、没有账号接入、没有托管遥测。真实用量数据只会导出到本地的 `data.js` 和 `data.raw.js`，并且这两个文件默认不会提交到 git。新版导出会把预计算视图放在 `data.js`；压缩字典和原始事件只会在自定义日期筛选时从 `data.raw.js` 按需加载。
...
Release zip 会内置预编译生成器，普通用户不需要安装 Go。启动脚本会先在本地生成 `data.js` 和 `data.raw.js`，然后打开 `index.html` 显示真实数据；源码 checkout 在没有预编译生成器时仍可回退到 `go build`。
...
生成器会把真实数据写到 `index.html` 同目录下的 `data.js` 和 `data.raw.js`。`data.js`、`data.raw.js` 和 `.codexscope-cache.json` 都可能包含项目名、会话 id、时间戳、用量模式和额度状态，所以已经被 `.gitignore` 排除。
...
- `generate_codex_data.go`：本地数据生成器，扫描 Codex JSONL 会话日志，提取用量元数据并写入 `data.js` 和 `data.raw.js`。
...
分享截图或导出的产物前，建议先检查自己的 `data.js` 和 `data.raw.js`。
