...
启动脚本会先在本地生成 `data.js`，然后打开 `index.html` 显示真实数据。
...
生成器会把真实数据写到 `index.html` 同目录下的 `data.js`。这个文件可能包含项目名、会话 id、时间戳、用量模式和额度状态，所以已经被 `.gitignore` 排除。
...
CodexScope 不会把数据发送到服务器。`generate_codex_data.py` 只读取本地 Codex 会话日志，并导出以下用量元数据：
